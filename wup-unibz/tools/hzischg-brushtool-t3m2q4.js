...
// CANVAS PRO - EVENT LISTENER VERSION (FIXED UI)
...
function createInterfaceAndBindEvents() {
  ...
  sidebar.innerHTML = `
    ...
    <div class="section">
      ...
      <div class="grid-tools">
        <button class="btn-tool active" id="btn-pencil" data-tool="pencil">${icons.pencil}<span>Matita</span></button>
        <button class="btn-tool" id="btn-pen" data-tool="pen">${icons.pen}<span>Penna</span></button>
        <button class="btn-tool" id="btn-marker" data-tool="marker">${icons.marker}<span>Marker</span></button>
        <button class="btn-tool" id="btn-spray" data-tool="spray">${icons.spray}<span>Spray</span></button>
        <button class="btn-tool" id="btn-watercolor" data-tool="watercolor">${icons.water}<span>Acqua</span></button>
        <button class="btn-tool" id="btn-oil" data-tool="oil">${icons.oil}<span>Olio</span></button>
        <button class="btn-tool" id="btn-eraser" style="grid-column: span 2" data-tool="eraser">${icons.eraser}<span>Gomma</span></button>
      </div>
    </div>

    <div class="section">
      <span class="label">Parametri <span id="size-val" class="val-disp">25px</span></span>
      <input type="range" id="size-input" min="1" max="100" value="25">
      
      <span class="label" style="margin-top:15px">Opacità <span id="opacity-val" class="val-disp">200</span></span>
      <input type="range" id="opacity-input" min="1" max="255" value="200">
      
      <span class="label" style="margin-top:15px">Colore</span>
      <div class="color-picker-wrap">
        <input type="color" id="color-input" value="#141414">
      </div>
    </div>
    ...
    <div class="section">
      <div style="display:flex; justify-content:space-between; align-items:center">
         ...
         <input type="checkbox" id="mirror-check" style="width:auto; margin:0">
      </div>
      <div id="mirror-options">
         <input type="range" id="mirror-count-input" min="2" max="12" value="6">
         <div style="font-size:10px; color:#666">Raggi: <span id="mirror-val">6</span></div>
      </div>
    </div>

    <div class="row-actions">
      <button class="btn-sec" id="btn-undo">Annulla</button>
      <button class="btn-sec" style="color:#ff5252; border-color:#522" id="btn-clear">Reset</button>
    </div>
    <div style="padding: 0 20px 20px 20px;">
      <button class="btn-primary" id="btn-save">Salva PNG</button>
    </div>
  `;
  document.body.appendChild(sidebar);

  // 4. BINDING DEGLI EVENTI (JS puro)
  
  // -- Tools Select --
  const tools = ['pencil', 'pen', 'marker', 'spray', 'watercolor', 'oil', 'eraser'];
  tools.forEach(t => {
  ...
  document.getElementById('size-input').addEventListener('input', (e) => {
     ...
     document.getElementById('size-val').innerText = brushSize + 'px';
  });
  
  document.getElementById('opacity-input').addEventListener('input', (e) => {
     brushOpacity = parseInt(e.target.value);
     document.getElementById('opacity-val').innerText = brushOpacity;
  });

  document.getElementById('color-input').addEventListener('input', (e) => {
     brushColor = color(e.target.value);
  });

  // -- Mirror --
  document.getElementById('mirror-check').addEventListener('change', (e) => {
     mirrorMode = e.target.checked;
     document.getElementById('mirror-options').style.display = mirrorMode ? 'block' : 'none';
  });
  
  document.getElementById('mirror-count-input').addEventListener('input', (e) => {
     mirrorCount = parseInt(e.target.value);
     document.getElementById('mirror-val').innerText = mirrorCount;
  });
  ...
  document.getElementById('btn-save').addEventListener('click', () => {
     saveCanvas('ArtWork_Pro', 'png');
  });
}

// Funzione Helper per cambio tool
function changeTool(t) {
  ...
  // Preset intelligenti
  let opInput = document.getElementById('opacity-input');
  ...
  else brushOpacity = 200; 

  opInput.value = brushOpacity;
