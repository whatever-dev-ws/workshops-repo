...
// TYPE TOOL V27 - EASY BACKGROUND PRESETS
...
let appState = {
  ...
  // Colore Testo (HSB Logic)
  hue: 180, sat: 100, bri: 100, alpha: 255,
  // Colore Sfondo (RGB Logic)
  bgColor: [30, 30, 35], 
  transp: false,
  // 3D & Effetti
  effect: 0, depth: 0,
...
function drawProGUI() {
  ...
  if(!appState.transp) {
    ...
    // 1. PRESETS RAPIDI (Pulsantini colorati)
    let gap = 5;
    ...
    for(let i=0; i<bgPresets.length; i++) {
       ...
       fill(c); 
       // Bordo bianco se attivo (approx check)
...
class OggettoTesto {
  ...
  getBoundingBox() {
    textSize(this.size); let w=0;
    if(this.effect!=='Geometrico') for(let i=0;i<this.content.length;i++) w+=textWidth(this.content[i])+this.spacing;
    else w=this.content.length*(this.size*0.6)+(this.content.length*this.spacing);
    return {w:w||20, h:this.size+20};
  }
  ...
  renderTxt(col) {
    if(this.effect==='Geometrico') {
      let pts=myFont.textToPoints(this.content,0,0,this.size,{sampleFactor:this.geoDensity, simplifyThreshold:0});
      ...
      let rad=map(this.geoDensity,0.05,0.5,8,2);
      for(let p of pts){ push(); translate(p.x,p.y); rotate(radians(appState.geoRot)); polygon(this,0,0,rad,appState.geoSides); pop(); }
      pop();
    } else {
      ...
      for(let i=0; i<this.content.length; i++) {
         push(); translate(cx+ws[i]/2, 0); this.eff(i); text(this.content[i],0,0); pop();
