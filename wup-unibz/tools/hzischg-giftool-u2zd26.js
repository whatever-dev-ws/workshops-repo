// WUP 2025/25
// Hannah Zischg
// VERSIONE SINGLE-FILE (NO HTML/CSS ESTERNI, NO P5.DOM)
...
let canvas; // Riferimento p5 al canvas
let mediaRecorder;
let recordedChunks = [];
...
function setup() {
  // 1. Inietta Stile CSS e crea l'Interfaccia HTML usando JS nativo
  createInterfaceJS();

  // 2. Crea il Canvas
  // Calcoliamo la larghezza disponibile (schermo meno sidebar)
  ...
  // 3. Posizionamento assoluto del Canvas per affiancarlo alla sidebar
  // Accediamo all'elemento HTML nativo del canvas
  let canvasEl = document.getElementById('p5Canvas');
  canvasEl.style.position = 'absolute';
  canvasEl.style.top = '0px';
  canvasEl.style.left = SIDEBAR_WIDTH + 'px';
  canvasEl.style.zIndex = '0'; // Dietro l'interfaccia se necessario
...
function draw() {
  ...
  if (params.trails) {
    ...
    // Pulisci Depth Buffer (Webgl hack per scie pulite)
...
// Queste funzioni devono essere attaccate a 'window' per essere viste dall'HTML iniettato
...
window.updateParam = function(key, val) {
  ...
  let label = document.getElementById('val-' + key);
  if(label) label.innerText = val;
}

window.setBool = function(key, isChecked) {
  params[key] = isChecked;
}

window.setRecordDuration = function(val) {
...
function syncUI() {
  ...
  keys.forEach(k => {
    let sl = document.getElementById('sld-' + k);
    let vl = document.getElementById('val-' + k);
    if(sl) sl.value = params[k];
    if(vl) vl.innerText = params[k];
  });
...
window.takeScreenshot = function() {
  saveCanvas(canvas, 'Design_3D', 'png');
}

window.startRecording = function() {
  recordedChunks = [];
  // Usa il selettore nativo per prendere il canvas
...
// --- COSTRUZIONE UI PURA (VANILLA JS) ---
// Questa parte sostituisce completamente p5.dom e il CSS esterno
...
function createInterfaceJS() {
  ...
  const css = `
    ...
    /* Reset Globale per la pagina */
    body, html { margin: 0; padding: 0; overflow: hidden; background: #0f0f14; width: 100%; height: 100%; }
    ...
    input[type="range"] { width: 100%; cursor: pointer; height: 4px; accent-color: #f50057; background: #333; border-radius: 2px; -webkit-appearance: none; }
    ...
    .btn-rec { background: #e0e0e0; color: #000; transition: background 0.3s; }
    .btn-shot { background: #222; color: #fff; border: 1px solid #333; }
    
    select { width: 100%; background: #222; color: #fff; border: 1px solid #333; padding: 6px; font-size: 11px; border-radius: 4px; outline: none; }
  `;
  ...
  sidebar.innerHTML = `
    ...
    <div class="section">
      ...
      ${makeHTMLSlider('Morph', 'morph', 0, 100, 20)}
      ${makeHTMLSlider('Twist', 'twist', 0, 100, 0)}
      ${makeHTMLSlider('Bulge', 'bulge', 0, 100, 0)}
      ${makeHTMLSlider('Noise', 'noise', 0, 100, 0)}
      ${makeHTMLSlider('Wave', 'wave', 0, 100, 30)}
      ${makeHTMLSlider('Esplodi', 'explode', 0, 200, 0)}
    </div>

    <div class="section">
      <span class="sec-title">Stile & Camera</span>
      ${makeHTMLSlider('Colore', 'hue', 0, 360, 330)}
      ${makeHTMLSlider('Zoom', 'zoom', 0.5, 3, 1, 0.1)}
      ${makeHTMLSlider('Velocità', 'rotSpeed', 0, 5, 1.5, 0.1)}
    ...
    <div class="section" style="border:none">
      ...
      <select onchange="window.setRecordDuration(this.value)">
      ...
      <div class="export-row">
        <button class="btn-exp btn-rec" id="btn-rec" onclick="window.startRecording()">${icons.video} REGISTRA</button>
        <button class="btn-exp btn-shot" onclick="window.takeScreenshot()">${icons.photo} FOTO</button>
      </div>
    </div>
  `;
...
function makeHTMLSlider(label, param, min, max, val, step=1) {
  return `
    <div class="control-row">
      ...
      <input type="range" id="sld-${param}" min="${min}" max="${max}" step="${step}" value="${val}" 
             oninput="window.updateParam('${param}', this.value)">
    </div>
  `;
}
