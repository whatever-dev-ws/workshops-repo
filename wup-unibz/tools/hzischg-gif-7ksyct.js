...
function setup() {
  // Impostazioni buffer per screenshot e video
  setAttributes('preserveDrawingBuffer', true);
  // Antialias disattivato a volte aiuta con le scie in WebGL, ma qui lo teniamo default
...
function draw() {
  // --- GESTIONE SCIE (FIX BUG) ---
  if (params.trails) {
    // 1. Disegna un piano semitrasparente davanti alla camera per l'effetto dissolvenza
    push();
    // Resetta la camera per disegnare in 2D sopra la scena 3D
    camera(0, 0, (height/2.0) / tan(PI*30.0 / 180.0), 0, 0, 0, 0, 1, 0);
    translate(0, 0, -100); // Sposta leggermente indietro
    noStroke();
    fill(15, 15, 20, 30); // Colore sfondo con opacità bassa
    plane(width*2, height*2); 
    pop();
    
    // 2. IMPORTANTE: Pulisci il Depth Buffer. 
    // Questo dice a WebGL: "Dimentica la profondità dei pixel precedenti, 
    // disegna la nuova forma sopra a tutto, anche se matematicamente sarebbe dietro".
    // Questo elimina il glitch grafico.
  ...
  if(params.fillShape) {
    ...
    // Luci per dare tridimensionalità
    ambientLight(60);
